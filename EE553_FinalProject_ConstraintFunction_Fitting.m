...
% Constraint Function Fitting
% Baris Canitez - 2033207

% Note-1: The optimum value is displayed in the command window and the
% optimum point is saved to the workspace with the handle "x_iter"
% Note-2: The code takes approximately 300 to 400 seconds to run due to large
% size. The variance of the timer is due to the randomized combinations of
% experiment matrix.
tic
clear variables
global D_low k_spring D_flapper F_preload
%% Forming the experiment matrix
exp_vec = linspace(0,13,14); %Expressing the normalized sampling intervals of the desicion variables with 14 points
...
%Pre-allocation for maximum stroke recorded for each model run
max_stroke = zeros(size(RealValue,1),1);

%For-loop for solving differential equation system for all sample points
%and recording maximum stroke values for each run
for i = 1:size(RealValue,1)
    ...
    max_stroke(i) = max(y(:,1)-y(:,5)); %Calculating maximum stroke from the maximum value between the displacamants of the drop mass and the unsrprung mass
end
%% Fitting of the Constraint Function
syms a1 a2 a3 %Defining elements a vector symbolically
a = [a1;a2;a3]; %Forming the a vector
% For loop for generating r vector
for i = 1:size(max_stroke,1)
    x_exp = RealValue(i,:).';
    r_store = max_stroke(i)-transpose(a)*x_exp;
    r(i,1) =r_store;
end
%Initial point where a is vector of ones with size of 3
x_initial = [1 1 1].';
vars = [a1 a2 a3].'; %Defining vector of variables for ease of substitution
...
%equation is in the form a1(i)*x1(i)+a2(i)*x2(i)+a3(i)*x3(i)
%Therefore, Jacobian w.r.t elements of a matrix is the minus of the sample point values directly
J = -RealValue;
...
global D_low D_flapper F_preload
% Distributing states
